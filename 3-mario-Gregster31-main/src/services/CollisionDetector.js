...
export default class CollisionDetector {
	...
	checkHorizontalCollisions(entity) {
		...
		if (entity.velocity.x > 0) {
			...
			if (this.isSolidTileInColumn(tileRight, tileTop, tileBottom)) {
				if (!this.isPlatformTileInColumn(tileRight, tileTop, tileBottom)) {
					// Collision on the right side (not a platform)
			...
			if (this.isSolidTileInColumn(tileLeft, tileTop, tileBottom)) {
				if (!this.isPlatformTileInColumn(tileLeft, tileTop, tileBottom)) {
					// Collision on the left side (not a platform)
	...
	isPlatformTileInColumn(column, top, bottom) {
		for (let y = top; y <= bottom; y++) {
			let tileId = this.map.getTileAt(Map.FOREGROUND_LAYER, column, y)?.id;
	...
	checkVerticalCollisions(entity) {
		...
		if (entity.velocity.y >= 0) {
			...
			if (
				...
				if (!this.isPlatformTileInRow(tileTop, tileLeft, tileRight)) {
					// Collision above
	...
	// Function to check if there are any platform tiles in the specified row.
	isPlatformTileInRow(row, left, right) {
		for (let x = left; x <= right; x++) {
			let tileId = this.map.getTileAt(Map.FOREGROUND_LAYER, x, row)?.id;
			if (tileId === 3 || tileId === 4) {
				return true; 
			}
		}
