...
export default class Map {
		...
		for (let y = 0; y < this.height; y++) {
			for (let x = 0; x < this.width; x++) {
				...
				switch (tileId) {
					case Tile.BLOCK:
						counter++;
						// First block in the map will have a mushroom inside it
						if (counter == 1)
						{
							this.blocks.push(
								new Block(
					...
					case Tile.COIN:
						...
						this.foregroundLayer.setTile(x, y, null); // Remove the coin tile
						continue;
					case Tile.MUSHROOM:
						this.mushrooms.push(
							new Mushroom(
	...
	update(dt) {
		...
		this.coins = this.coins.filter((coin) => !coin.isCollected); // Filters out collected coins 

		this.mushrooms.forEach((mushroom) => mushroom.update(dt));
		this.mushrooms = this.mushrooms.filter((mushroom) => mushroom.isAlive);
